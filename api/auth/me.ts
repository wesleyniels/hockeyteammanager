...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'PUT') {
    const body = req.body ?? {}
    for (const key of ['defaultTeam', 'firstName', 'lastName', 'role']) {
      if (body[key] !== undefined && body[key] !== null && typeof body[key] !== 'string') {
        res.status(400).json({ error: `Invalid ${key}` })
        return
      }
    }
    const current = await sql`SELECT default_team, first_name, last_name, role FROM users WHERE id = ${session.id}`
