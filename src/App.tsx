...
// ── One-time fixture import: SC Muiden's youth teams ────────────────────────────
// This app's own roster-naming convention doesn't always match hockey.nl's team
// numbering (jersey-color names like MO9-Blauw/MO9-Geel vs. hockey.nl's
// MO9-1/MO9-2) — most teams happen to share a name, but MO11-Blauw is
// hockey.nl's MO11-2, and JO11-Blauw is JO11-1. There's no live sync with
// hockey.nl (its robots.txt disallows automated /api/ access, which almost
// certainly backs the match-center SPA), so this is a fixed, one-time
// transcription of each team's KNHB voorcompetitie schedule (match center,
// read 2026-08-11) for whichever poule it had fixtures in at the time.
// MO7-Blauw/MO7-Geel/MO7-Rood and JO7-Blauw (hockey.nl: JO7-Wit) had no
// schedule published yet for that age group. Re-transcribe by hand next
// season rather than building a standing scraper.
const TEAM_FIXTURES: Record<string, { date: string; opponent: string; homeAway: 'Thuis' | 'Uit' }[]> = {
  'MO7-Blauw': [],
  'MO7-Geel': [],
  'MO7-Rood': [],
  'MO8-Blauw': [
...
export default function App() {
  ...
  // Fires once whoever's using the app picks a team in Setup that has an
  // entry in TEAM_FIXTURES, creating any of that team's fixtures they don't
  // already have a game for. Deferred to an effect (rather than running
  // inline from the dropdown's own callback) because `games` may still be
  // mid-fetch right when the team gets picked — e.g. the profile-default
  // auto-select fires the moment `user` loads, which can easily race the
  // separate games request. The per-fixture existence check (not a
  // one-time flag) is what actually makes this idempotent: safe to fire on
  // every selection, on any device, forever.
  const [pendingTeamForImport, setPendingTeamForImport] = useState<string | null>(null)
  useEffect(() => {
    if (gamesLoading || !pendingTeamForImport) return
    const fixtures = TEAM_FIXTURES[pendingTeamForImport]
    if (fixtures) {
      const ageGroup = ageGroupFromTeamName(pendingTeamForImport)
      for (const fx of fixtures) {
        const exists = games.some(g => g.team === pendingTeamForImport && g.date === fx.date && g.opponent === fx.opponent)
        if (exists) continue
        addGame({
          id: uid(), date: fx.date, club: 'SC Muiden', team: pendingTeamForImport, ageGroup,
